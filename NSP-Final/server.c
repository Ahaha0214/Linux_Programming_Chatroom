...
void worker_process(int worker_id) {
    ...
    while (1) {
        ...
        // Optimization: Read version without lock first (atomic on 64bit usually, but let's be safe-ish or just lock)
        // Actually, reading a 64bit int might tear on 32bit systems, but on 64bit Linux it's fine.
        // Let's just lock briefly to get version.
        pthread_mutex_lock(&game_state->lock);
            ...
            if (i != server_fd && FD_ISSET(i, &masterfds)) {
                if (client_ids[i] != -1) {
                    ...
                    if (client_versions[i] < current_version) {
                        // Send Update
                        // We need to lock to read the map safely? Yes.
                        // But locking for every client send is slow.
                        // Copy map to local buffer?
                        // Let's lock, copy, unlock.
                        ...
                        if (send_packet(i, OP_UPDATE, map_copy, sizeof(map_copy)) == -1) {
                             ...
                             // Let next loop handle it via read failure
                        } else {
